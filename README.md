...
├── install-n8n.sh        # n8n 초기 설치 스크립트 (라이센스 포함)
...
### install-n8n.sh - n8n 초기 설치 (라이센스 포함)

**용도**: n8n을 처음 설치할 때 라이센스와 함께 실행합니다.

**사용 시점**: 
- 최초 1회 설치 시
- 볼륨을 완전히 초기화하고 새로 설치할 때

**주요 기능**:
1. 볼륨 생성 (기존 볼륨이 있으면 삭제 확인)
2. 라이센스 환경 변수와 함께 n8n 실행
3. 라이센스가 볼륨에 저장됨
...
다음부터는 ./run-n8n.sh를 사용하여 실행하세요.
```

**주의사항**:
- 기존 볼륨이 있으면 삭제 여부를 확인합니다
- 볼륨 삭제 시 모든 데이터가 사라집니다
- 라이센스는 볼륨에 저장되므로 이후 실행 시 환경 변수 불필요
